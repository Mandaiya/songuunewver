...
def welcomepic(pic, user, chatname, id, uname):
    background = Image.open("ANNIEMUSIC/assets/annie/Welcomeme.png")
    pfp = Image.open(pic).convert("RGBA")
    pfp = circle(pfp)
    pfp = pfp.resize((835, 839))
    draw = ImageDraw.Draw(background)
    font_large = ImageFont.truetype('ANNIEMUSIC/assets/annie/ArialReg.ttf', size=65)
    font_small = ImageFont.truetype('ANNIEMUSIC/assets/annie/ArialReg.ttf', size=1)
    draw.text((421, 715), f'{user}', fill=(0, 0, 0), font=font_small)
    draw.text((270, 1005), f'{id}', fill=(0, 0, 0), font=font_small)
    draw.text((570, 1308), f"{uname}", fill=(0, 0, 0), font=font_small)
